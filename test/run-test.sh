...
function run_code_tests {
    ...
    run_test_fail "code contains path as string \"/path/\"" "grep -RPnr --include='*.dm' '\"/(obj|datum|atom|turf|area|mob)/[^\[\s\(]*\"' code/"
    run_test_fail "changed files contains proc argument starting with 'var'" "grep -RPnr --include='*.dm' '^/[\w/]\S+\(.*(var/|, ?var/.*).*\)' code/"
