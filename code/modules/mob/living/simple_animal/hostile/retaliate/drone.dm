...
/mob/living/simple_animal/hostile/retaliate/malf_drone/Life()
	...
	if(prob(disabled ? 0 : 1))
		if(hostile_drone)
			visible_message("<span class='notice'>[bicon(src)] [src] retracts several targetting vanes, and dulls it's running lights.</span>")
			hostile_drone = 0
		else
			visible_message("<span class='warning'>[bicon(src)] [src] suddenly lights up, and additional targetting vanes slide into place.</span>")
			hostile_drone = 1
	...
	if(health / maxHealth > 0.9)
		...
		if(!disabled)
			if(prob(50))
				...
				visible_message("<span class='notice'>[bicon(src)] [src] suddenly lies still and quiet.</span>")
